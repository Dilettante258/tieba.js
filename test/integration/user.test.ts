...
describe("User APIs", () => {
	...
	test("getUserByUid — 返回用户信息", async () => {
		const data = await safeRun(
			getUserByUid(TEST_PARAMS.tiebaUid),
		);
		if (!data) return;
	...
	test("getProfile — 通过 portrait 查询", async () => {
		const data = await safeRun(
			getProfile(TEST_PARAMS.portrait),
		);
		if (!data) return;
		expect(data).toBeDefined();
		if (data.user) {
	...
	test("getLikeForum — 返回关注的吧列表", async () => {
		const data = await Effect.runPromise(
			getLikeForum(TEST_PARAMS.userId),
		);
	...
	test("getFollow — 返回关注列表", async () => {
		const data = await Effect.runPromise(
			getFollow(TEST_PARAMS.userId, 1),
		);
