...
import {FirstPostContent, OutputPostList, PostList, PostListContent, RawUserPost, Thread, UserPost} from "../types";
...
let defaultBDUSS: string = "";
let needPlainText: boolean = false;
...
export function collatePost(posts: PostList[], withComment = true): OutputPostList[] {
  let result: OutputPostList[] = [];
  for (let post of posts) {
    let temp: OutputPostList = {
      ...post,
      signature: post?.signature ? processContent(post.signature.content) : undefined,
      subPostList: undefined,
      content: processContent(post.content),
    }
    if (withComment && post.subPostList) {
      temp.subPostList = post.subPostNumber > 0 ? post.subPostList.subPostList.map(subPost => {
        if (subPost.content.length >= 3 && subPost.content[1]?.uid !== undefined) {
          return {
            id: subPost.id,
            time: subPost.time,
            ...
            content: processContent(subPost.content.slice(2)).substring(2),
          }
        } else {
          return {
            id: subPost.id,
